...
def train_model(model, opt):
    ...
    for epoch in range(opt.epochs):
        ...
        for i, batch in enumerate(opt.train):
            ...
            preds, _ = model(mfcc=mfcc, input_ids=input_ids, attention_mask=attention_mask, mfcc_mask=None,
                             src_mask=src_mask)  # (B，length, 3)
...
def main():
    ...
    parser.add_argument("--key", default='ppg', type=str)
    ...
    parser.add_argument('-ppg_path', type=str,
                        default='./ppg_features')
    ...
    train_data = Load_audio_and_text_data(dataset=train_dataset, bert_embedding_length=opt.bert_embedding_length, p1=opt.ppg_path)
    dev_data = Load_audio_and_text_data(dataset=dev_dataset, bert_embedding_length=opt.bert_embedding_length, p1=opt.ppg_path)
