...
class Load_audio_and_text_data(Dataset):
    def __init__(self, dataset, bert_embedding_length, p1):
        self.data = dataset
        self.p1 = p1
        self.bert_embedding_length = bert_embedding_length

    def __getitem__(self, index):
        attention_mask = torch.as_tensor(self.data[index]['attention_mask'])
        input_ids = torch.as_tensor(self.data[index]['input_ids'])
        labels = self.data[index]['labels']
        id = self.data[index]['id']
        org_len = len(labels)
        padded_labels = [int(labels[i]) if i < org_len else 0 for i in range(0, self.bert_embedding_length)]
        padded_labels = torch.as_tensor(padded_labels)
        temp = eval(self.data[index]['text'])['source']
        source = ''.join(temp)
        return attention_mask, input_ids, padded_labels, org_len, source, id
